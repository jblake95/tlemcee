...
def simulateEphem(site, norad_id, start, end, step, out_dir=None):
    """
    ...
    for t, time in enumerate(times):
        print('Calculating predicted positions: {}/{}'.format(t,
                                                              len(times)), end="\r")
        ephem.update('UTC', t, time.isoformat())

        predicted = tle.radec_at(time,
                                 site)
        ephem.update('RA', t, predicted.ra.value)
        ephem.update('DEC', t, predicted.dec.value)

        ha = tle.hourangle_at(time,
                              site)
        ephem.update('HA', t, ha.hourangle)

        predicted = tle.altaz_at(time,
                                 site)
        ephem.update('ALT', t, predicted.alt.value)
        ephem.update('AZ', t, predicted.az.value)
